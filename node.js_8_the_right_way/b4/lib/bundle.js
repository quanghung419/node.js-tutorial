...
module.exports = (app, es) => {
    ...
    app.put('/api/bundle/:id/book/:pgid', async (req, res) => {
        ...
        try {
            ...
            const idx = bundle.books.findIndex(
    ...
    app.delete('/api/bundle/:id/book/:pgid', async (req, res) => {
        const bundleUrl = `${url}/${req.params.id}`;
        try {
            // Retrieve list books of bundle by bundle id
            const options = {
                url: bundleUrl,
                json: true
            };
            const bundleRes = await rp(options);
            const { _source: bundle, _version: version } = bundleRes;
            // Delete a book by id
            for (var i = 0; i < bundle.books.length; i++) {
                const book = bundle.books[i];
                if (book.id && book.id === req.params.pgid) {
                    bundle.books.splice(i, 1);
                }
            }
            // Put the updated bundle back in the index.
            const esResBody = await rp.put({
